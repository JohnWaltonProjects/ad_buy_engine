use crate::db::invitation_depricating;
use crate::db::invitation_depricating::remove;
...
pub async fn create(
    ...
    block(move || crate::db::invitation_depricating::dedup(&_pool, params.email.clone()))
        .await
        .map_err(|e| {
...
pub async fn update(_id: Path<Uuid>, pool: Data<PgPool>) -> Result<HttpResponse, ApiError> {
    ...
    if item.expires_at > ad_buy_engine::chrono::Local::now().naive_local() {
        ...
        block(move || invitation_depricating::update(&pool_a, &item)).await?;
