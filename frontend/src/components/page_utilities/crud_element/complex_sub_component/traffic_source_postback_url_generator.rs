...
pub struct Props {
    #[prop_or_default]
    pub tokens: Vec<DataURLToken>,
    #[prop_or_default]
    pub url: Option<Url>,
    pub eject: Callback<Url>,
}

pub struct TrafficSourcePostbackUrlGenerator {
...
impl Component for TrafficSourcePostbackUrlGenerator {
    type Message = Msg;
    ...
    fn change(&mut self, props: Self::Properties) -> ShouldRender {
        ...
        if self.props.tokens.len() != props.tokens.len() {
            let more_difference = props.tokens.iter().find(|s| !self.props.tokens.contains(s));

            let less_difference = self.props.tokens.iter().find(|s| !props.tokens.contains(s));

            if props.tokens.len() > self.props.tokens.len() {
