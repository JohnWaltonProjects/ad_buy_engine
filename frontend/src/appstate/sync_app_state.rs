...
use ad_buy_engine::data::elements::sync::{
    SyncElementRequest, SyncElementResponse, SyncElementUnit, SyncFlagData,
};
use yew::format::Json;
            ...
            match flag {
                SyncFlagData::Update(item) => {
                    if let Some(pos) = local_state
                        .iter()
                        .position(|s| s.offer_source_id == item.offer_source_id)
                    {
        ...
        for flag in sync_response.traffic_sources {
            ...
            match flag {
                SyncFlagData::Update(item) => {
                    if let Some(pos) = local_state
                        .iter()
                        .position(|s| s.traffic_source_id == item.traffic_source_id)
                    {
                        local_state.remove(pos);
                        local_state.insert(pos, item);
                    }
        ...
        for flag in sync_response.landers {
            ...
            match flag {
                SyncFlagData::Update(item) => {
                    if let Some(pos) = local_state
                        .iter()
                        .position(|s| s.landing_page_id == item.landing_page_id)
                    {
                        local_state.remove(pos);
                        local_state.insert(pos, item);
                    }
        ...
        for flag in sync_response.funnels {
            ...
            match flag {
                SyncFlagData::Update(item) => {
                    if let Some(pos) = local_state
                        .iter()
                        .position(|s| s.funnel_id == item.funnel_id)
                    {
                        local_state.remove(pos);
                        local_state.insert(pos, item);
                    }
        ...
        for flag in sync_response.campaigns {
            ...
            match flag {
                SyncFlagData::Update(item) => {
                    if let Some(pos) = local_state
                        .iter()
                        .position(|s| s.campaign_id == item.campaign_id)
                    {
                        local_state.remove(pos);
                        local_state.insert(pos, item);
                    }
