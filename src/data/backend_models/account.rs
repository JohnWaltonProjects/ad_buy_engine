...
use crate::data::account::{
    Account, ConversionRegistrationTimeReporting, DefaultHomeScreen, DefaultWayToOpenReport,
...
impl From<Account> for AccountModel {
    fn from(account: Account) -> Self {
        Self {
            ...
            billing_currency: serde_json::to_string(&account.default_reporting_currency)
                .expect("rerRg"),
            sys_language: serde_json::to_string(&account.sys_language).expect("rerRg"),
            domains_configuration: serde_json::to_string(&account.domains_configuration)
                .expect("rerRg"),
            work_spaces: serde_json::to_string(&account.work_spaces).expect("rerRg"),
            fuel: serde_json::to_string(&account.fuel).expect("rerRg"),
            conversion_registration_time_reporting: serde_json::to_string(
                &account.conversion_registration_time_reporting,
            )
            .expect("rerRg"),
            default_home_screen: serde_json::to_string(&account.default_home_screen)
                .expect("rerRg"),
            default_way_to_open_report: serde_json::to_string(&account.default_way_to_open_report)
                .expect("rerRg"),
            ip_anonymization: account.ip_anonymization,
            default_reporting_currency: serde_json::to_string(&account.default_reporting_currency)
                .expect("rerRg"),
            ...
            two_factor_authentication: serde_json::to_string(&account.two_factor_authentication)
                .expect("rerRg"),
            api_access_keys: serde_json::to_string(&account.api_access_keys).expect("rerRg"),
            billing_information: serde_json::to_string(&account.billing_information)
                .expect("rerRg"),
            custom_conversions: serde_json::to_string(&account.custom_conversions).expect("rerRg"),
            referrer_handling_list: serde_json::to_string(&account.referrer_handling_list)
                .expect("rerRg"),
...
impl From<AccountModel> for Account {
    fn from(account_model: AccountModel) -> Self {
        Self {
            ...
            report_time_zone: serde_json::from_str(&account_model.report_time_zone)
                .expect("GV545r3"),
            billing_currency: serde_json::from_str(&account_model.billing_currency)
                .expect("GV54fg3"),
            sys_language: serde_json::from_str(&account_model.sys_language).expect("G4V54fg3"),
            domains_configuration: serde_json::from_str(&account_model.domains_configuration)
                .expect("GV54fg4re3"),
            ...
            conversion_registration_time_reporting: serde_json::from_str(
                &account_model.conversion_registration_time_reporting,
            )
            .expect("G4V54fg35"),
            default_home_screen: serde_json::from_str(&account_model.default_home_screen)
                .expect("GV54fgfd3"),
            default_way_to_open_report: serde_json::from_str(
                &account_model.default_way_to_open_report,
            )
            .expect("GV5455fg3"),
            ip_anonymization: account_model.ip_anonymization,
            default_reporting_currency: serde_json::from_str(
                &account_model.default_reporting_currency,
            )
            .expect("GV54fggfg3"),
            ...
            additional_users: serde_json::from_str(&account_model.additional_users)
                .expect("GV54f4efg3"),
            skype: account_model.skype,
            phone_number: account_model.phone_number,
            two_factor_authentication: serde_json::from_str(
                &account_model.two_factor_authentication,
            )
            .expect("GV54fdfdfg3"),
            api_access_keys: serde_json::from_str(&account_model.api_access_keys)
                .expect("df4GV54fg3"),
            billing_information: serde_json::from_str(&account_model.billing_information)
                .expect("GV54fggf45g53"),
            custom_conversions: serde_json::from_str(&account_model.custom_conversions)
                .expect("GV54fgvf3"),
            referrer_handling_list: serde_json::from_str(&account_model.referrer_handling_list)
                .expect("GV54fgfgfg3"),
            last_updated: DateTime::<Utc>::from_utc(
                NaiveDateTime::from_timestamp(account_model.last_updated, 0),
                Utc,
            ),
        }
    }
